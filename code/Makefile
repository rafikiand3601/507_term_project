...
SOURCES = main.cpp task_user.cpp adc.cpp task_steering.h
...
# Standard fuses FF19EF, bootloader fuses FFC8EF, low power fuses FF11EF
  ...
  LFUSE = 0xEF
# ATmega2561
# Standard fuses from http://www.engbedded.com/fusecalc/
...
install:  $(ELF) $(BIN) $(HEX)
  ...
  else ifeq ($(PROG), usbasp)
	$(DUDE) -c usbasp -P usb -p $(MCU) -V -B 1 -Uflash:w:$(HEX)
  else
...
fuses:
  ifeq ($(PROG), bsd)
	...
	$(DUDE) -p $(MCU) -c usbasp -q -V -Ulfuse:w:$(LFUSE):m
	$(DUDE) -p $(MCU) -c usbasp -q -V -Uhfuse:w:$(HFUSE):m
	$(DUDE) -p $(MCU) -c usbasp -q -V -Uefuse:w:$(EFUSE):m
  else
...
readfuses:
  ifeq ($(PROG), bsd)
	...
	$(DUDE) -c usbasp -p $(MCU) -U lfuse:r:low_fuse_val.hex:h -U hfuse:r:high_fuse_val.hex:h
  else
